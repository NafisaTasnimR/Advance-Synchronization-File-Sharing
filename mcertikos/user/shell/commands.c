...
err_t cmd_flocktest(int argc, char **argv, int optc, char **optv)
{
    pid_t pid = spawn(6, 256);
    if (pid == -1) {
        printf("Failed to launch flocktest.\n");
        return E_CANT_OPEN;
    }
    printf("flocktest running in process %d\n", pid);
    return E_OK;
}

err_t cmd_fstest(int argc, char **argv, int optc, char **optv)
{
    pid_t pid = spawn(4, 256);
    if (pid == -1) {
        printf("Failed to launch fstest.\n");
        return E_CANT_OPEN;
    }
    printf("fstest running in process %d\n", pid);
    return E_OK;
}
