...
static uint64_t tsc_calibrate(uint32_t latch, uint32_t ms, int loopmin)
{
    ...
    while ((inb(0x61) & 0x20) == 0) {
        ...
        if (delta > tscmax)
            tscmax = delta;
        pitcnt++;
        /* Safety: if PIT gate never toggles (e.g. TCG), bail out */
        if (pitcnt > 10000000)
            return ~(uint64_t) 0x0;
    }
...
int tsc_init(void)
{
    ...
    /*
     * Try PIT-based TSC calibration. On TCG (-icount) the PIT channel 2
     * output gate may never toggle, so cap the loop to avoid an infinite spin.
     */
    ret = ~(uint64_t) 0x0;
    for (i = 0; i < 3; i++) {
