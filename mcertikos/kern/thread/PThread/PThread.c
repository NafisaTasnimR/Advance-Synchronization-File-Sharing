...
void thread_sleep(void *chan, spinlock_t *lk)
{
    ...
    // Tidy up. Avoid holding sched_lk while reacquiring lk to prevent
    // lock inversion deadlocks with code that calls thread_wakeup() while
    // holding lk (a common pattern in this codebase).
    spinlock_acquire(&sched_lk);
