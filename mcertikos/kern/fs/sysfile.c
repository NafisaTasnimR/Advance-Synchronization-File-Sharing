...
static char fsbuf[NUM_IDS][BUFLEN_MAX];
...
{
    if (!(VM_USERLO <= buf && buf + len <= VM_USERHI) || (0 < maxlen && maxlen <= len))
    {
...
void sys_read(tf_t *tf)
{
    ...
    read = file_read(file, fsbuf[pid], buflen);
    syscall_set_retval1(tf, read);
    if (0 <= read)
    {
        pt_copyout(fsbuf[pid], pid, buf, read);
...
void sys_write(tf_t *tf)
{
    ...
    pt_copyin(pid, buf, fsbuf[pid], buflen);
    written = file_write(file, fsbuf[pid], buflen);
 ...
 * Validates fd, retrieves file from process TCB, delegates to file_flock().
 * Bridges syscall layer to file system layer.
 *
 * Return Value: 0 on success, -1 on failure
 * Error codes: E_BADF (bad fd), E_INVAL_ADDR (operation would block with LOCK_NB)
 */
void sys_flock(tf_t *tf)
{
    ...
    result = file_flock(file, operation);
    ...
    {
        syscall_set_errno(tf, E_INVAL_ADDR); // or E_BADF depending on error
        syscall_set_retval1(tf, -1);
    }
}
