...
/**
 * Apply or remove a lock on file f.
 * Handles lock acquisition/release logic, validates file type,
 * manages lock upgrades, calls core flock engine.
 */
int file_flock(struct file *f, int operation)
{
    ...
    if (f->type != FD_INODE)
    {
        return -1; // Can only lock regular files
    }

    // Handle unlock operation
    if (lock_type == LOCK_UN)
    {
        if (f->holding_flock != 0)
        {
            result = flock_release(&f->ip->flock, f->holding_flock);
            if (result == 0)
            {
                f->holding_flock = 0;
            }
        ...
        return 0; // Not holding a lock, nothing to unlock
    }

    // Handle lock upgrade/downgrade
    if (f->holding_flock != 0)
    {
        // If already holding the same type of lock, just return success
        if (f->holding_flock == lock_type)
        {
            return 0;
        }
        // Release old lock before acquiring new one (upgrade/downgrade)
        flock_release(&f->ip->flock, f->holding_flock);
        f->holding_flock = 0;
    }
